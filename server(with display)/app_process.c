...
void tx_req()
{
  //app_log_info("callback run \n");
  tx_requested = true;
}
void request_tx(RAIL_Handle_t rail_handle)
{
  if (!RAIL_IsTimerRunning(rail_handle))
  {
    RAIL_SetTimer(rail_handle, 1000000, RAIL_TIME_DELAY , tx_req);
    //app_log_info("setting timer \n");
  }
  else
    {
      //app_log_info("timer already set");
    }
...
static void handle_packet_transmission(RAIL_Handle_t rail_handle)
{
  ...
  if(ack_count == 1)
  {
      if (sl_led_get_state(&sl_led_led1)==1);
      sl_led_turn_off(&sl_led_led1);
  }
  if (ack_count >= 2)
  {
//      char buffer[1024];
//      snprintf(buffer, sizeof(buffer), "Bad transmition, warning");
...
static void handle_received_packet(RAIL_Handle_t rail_handle)
{
  ...
  if (rx_packet_handle != RAIL_RX_PACKET_HANDLE_INVALID) {
    ...
    if (packet_details.isAck) {
      ...
      app_log_info("RSSI: %d, LQI: %d \n", packet_details.rssi, packet_details.lqi);
      if(request_calibrate)
      {
         model_rssi = packet_details.rssi;
         request_calibrate = false;
         app_log_info("new model RSSI: %d \n", model_rssi);
      }
      if (true) {
        char buffer[1024];
        snprintf(buffer, sizeof(buffer), "RSSI: %d \nEst.Distance: %.2f m ", packet_details.rssi, pow(10, (model_rssi - packet_details.rssi)/(10*5)));
        print_to_lcd(buffer);
      }
    } else {
